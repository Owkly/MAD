...
```{r}
nb_true <- sum(data_combined$remote_allowed)
nb_total <- length(data_combined$remote_allowed)

# Calcul du pourcentage de TRUE
pourcentage_true <- (nb_true / nb_total) * 100
print(paste("Pourcentage de TRUE :", pourcentage_true, "%"))
```
...
```{r}
nb_true <- sum(data_combined$sponsored)
nb_total <- length(data_combined$sponsored)
